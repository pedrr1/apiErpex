...
class UserControl
{
    ...
    public function deleteDeviceUser(): void
    {
        try {
            ...
            $start = microtime(true);
            $this->service->deleteDevice($body['IdDevice']);
            $duration = (int)((microtime(true) - $start) * 1000);
            $this->logSucess($this->service, 'deleteDevice', $service, $duration, $traceId);

            $start = microtime(true);
            $this->response->userResponse(['message' => 'Dispositivo deletado com sucesso']);
            $duration = (int)((microtime(true) - $start) * 1000);
            $this->logSucess($this->response, 'userResponse', $service, $duration, $traceId);
    ...
    public function getDeviceUser(): void
    {
        try {
            $this->request = new UserRequest();
            $traceId = bin2hex(random_bytes(16));
            ...
            $start = microtime(true);
            $devices = $this->service->getDevices($body['IdRequest'], $body['IdDevice']);
            $duration = (int)((microtime(true) - $start) * 1000);
            $this->logSucess($this->service, 'getDevices', $service, $duration, $traceId);

            $start = microtime(true);
            $this->response->userResponse($devices);
            $duration = (int)((microtime(true) - $start) * 1000);
            $this->logSucess($this->response, 'userResponse', $service, $duration, $traceId);
    ...
    public function checkAddDevices(): void{
        try {
            $this->request = new UserRequest();
            $traceId = bin2hex(random_bytes(16));
            ...
            $start = microtime(true);
            $this->service->checkAddDevices($body['IdRequest']);
            $duration = (int)((microtime(true) - $start) * 1000);
            $this->logSucess($this->service, 'checkAddDevices', $service, $duration, $traceId);

            $start = microtime(true);
            $this->response->userResponse(['message' => 'Verificação de dispositivos concluída']);
            $duration = (int)((microtime(true) - $start) * 1000);
            $this->logSucess($this->response, 'userResponse', $service, $duration, $traceId);
    ...
    public function addDevicesUser(): void
    {
        try {
            ...
            $start = microtime(true);
            $this->service->addDevicesUser($body['IdRequest'], $body['IdDevice'], $body['NameDevice'] ?? 'Meu Dispositivo');
            $duration = (int)((microtime(true) - $start) * 1000);
     ...
     public function deleteUser(): void
    {
        try {
            ...
            $start = microtime(true);
            $this->service->deleteUser($body['IdRequest']);
            $duration = (int)((microtime(true) - $start) * 1000);
            $this->logSucess($this->service, 'deleteUser', $service, $duration, $traceId);

                $start = microtime(true);            
            $this->response->userResponse(['message' => 'Usuario deletado com sucesso']);
            $duration = (int)((microtime(true) - $start) * 1000);
            $this->logSucess($this->response, 'userResponse', $service, $duration, $traceId);
